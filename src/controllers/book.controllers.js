...
export const crearLibro = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "no puede ser undefined, escriba algo porfavor!" });
    ...
    if (title === "" || author === "" || genre === "" || pages === "")
      return res
        .status(400)
        .json({
          message: "el campo no puede estar vacio, escriba algo porfavor!",
        });
    ...
    if (page < 0)
      return res
        .status(400)
        .json({ message: "no puede ser un numero negativo" });
  } catch (error) {
...
export const traerLibrosId = async (req, res) => {
  try {
    ...
    res.json(libros);
    return res.status(400).json({ message: "no se pudo traer los libros" });
  } catch (error) {
    return res.status(404).json({ message: "Error por parte del servidor" });
  }
};
...
export const actualizarLibro = async (req, res) => {
  try {
    const { updated } = await libros.update({
